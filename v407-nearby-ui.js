(() => {
  ...
  function updateActive(r){
    document.querySelectorAll('[data-dz-radius]').forEach(b=>b.classList.toggle('active',Number(b.dataset.dzRadius)===Number(r)));
  ...
  function stopCoords(row){
    const m=row.operator==='KMB'?state.kmbStops:row.operator==='CTB'?state.ctbStops:state.gmbStops;
    const s=m?.get(String(row.stopId)); if(!s)return null;
    const lat=Number(s.lat??s.latitude),lon=Number(s.long??s.lng??s.longitude);
    return Number.isFinite(lat)&&Number.isFinite(lon)?{lat,lon,name:s.name_tc||row.stopName||row.route}:null;
  }
  function ensureHost(){
    let host=$('#dz407Map'); if(host)return host;
    host=document.createElement('section');host.id='dz407Map';host.className='dz407-map hidden';host.innerHTML='<div id="dz407MapCanvas"></div>';
    const sec=$('#nearbySection'); if(sec)sec.appendChild(host); return host;
  }
  async function showMap(){
    const host=ensureHost(),btn=$('#dz407MapBtn');
    if(!host)return;
    if(!host.classList.contains('hidden')){host.classList.add('hidden');if(btn)btn.textContent='地圖';return;}
    if(btn){btn.disabled=true;btn.textContent='地圖載入中…';}
    try{
      const L=await ensureLeaflet();
      const pos=await new Promise((resolve,reject)=>navigator.geolocation.getCurrentPosition(p=>resolve({lat:p.coords.latitude,lon:p.coords.longitude}),reject,{enableHighAccuracy:true,maximumAge:8000,timeout:12000}));
      host.classList.remove('hidden');
      if(!map){map=L.map($('#dz407MapCanvas'),{zoomControl:true});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map);layer=L.layerGroup().addTo(map);}else layer.clearLayers();
      if(userMarker)map.removeLayer(userMarker); userMarker=L.marker([pos.lat,pos.lon]).addTo(map).bindTooltip('你的位置');
      const pts=[[pos.lat,pos.lon]];
      for(const r of (state.nearby||[]).slice(0,24)){const c=stopCoords(r);if(!c)continue;pts.push([c.lat,c.lon]);L.marker([c.lat,c.lon]).addTo(layer).bindTooltip(`${r.route} · ${c.name}`);}
      if(pts.length>1)map.fitBounds(pts,{padding:[20,20],maxZoom:18});else map.setView([pos.lat,pos.lon],17);
      setTimeout(()=>map.invalidateSize(),80); if(btn)btn.textContent='收起地圖';
    }catch{if(btn)btn.textContent='地圖';}
    finally{if(btn)btn.disabled=false;}
  }
  window.addEventListener('click',e=>{
    const r=e.target.closest?.('[data-dz-radius]'); if(r){updateActive(Number(r.dataset.dzRadius)||100); return;}
    if(e.target.closest?.('#dz407MapBtn')){e.preventDefault();e.stopImmediatePropagation();showMap();}
  },true);
  ...
  installControls();
  const badge=$('.app-version');if(badge){badge.textContent=`v${VERSION}`;badge.setAttribute('aria-label',`版本 v${VERSION}`);}window.dzNearbyUI407={version:VERSION,showMap};
})();