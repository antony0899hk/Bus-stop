(() => {
  ...
  async function renderNearbyMap(){
    ...
    if(!state393.target||!state393.stops.some(x=>x.stop===state393.target.stop&&x.operator===state393.target.operator))state393.target=state393.stops[0]||null;drawWalk();focusRadius();startWatch();setTimeout(()=>map.invalidateSize(),80);
  }
