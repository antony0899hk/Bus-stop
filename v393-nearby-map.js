(() => {
  ...
  const state393={radius:100,position:null,heading:null,lastPosition:null,stops:[],target:null,map:null,userMarker:null,radiusCircle:null,walkLine:null,walkArrow:null,stopLayers:[],watchId:null,leafletPromise:null};
  window.dzNearbyMapState=state393;

  function ensureControls(){
    const panel=$(".nearby-panel");if(!panel||panel.querySelector(".dz393-radius-row"))return;
  ...
  function mapHost(){
    let host=$("#dz393NearbyMap");if(host)return host;
    host=document.createElement("section");host.id="dz393NearbyMap";host.className="dz393-nearby-map hidden";host.innerHTML='<div class="dz393-map-head"><div><strong>附近車站地圖</strong><span id="dz393MapMeta">即時位置</span></div><button type="button" id="dz393Recenter">◎</button></div><div id="dz393MapCanvas" class="dz393-map-canvas"></div>';
    const nearby=$("#nearbySection");nearby?.insertAdjacentElement("afterbegin",host);return host;
  }
  async function ensureLeaflet(){
    ...
    state393.leafletPromise=new Promise((resolve,reject)=>{
      ...
      const old=document.querySelector('script[data-dz-leaflet]');if(old){old.addEventListener('load',()=>resolve(window.L),{once:true});old.addEventListener('error',()=>reject(new Error('地圖元件載入失敗')),{once:true});return;}
  ...
  function bearing(a,b){
    if(!a||!b)return 0;const toRad=x=>x*Math.PI/180,toDeg=x=>x*180/Math.PI;const p1=toRad(a.lat),p2=toRad(b.lat),dl=toRad(b.lon-a.lon);const y=Math.sin(dl)*Math.cos(p2),x=Math.cos(p1)*Math.sin(p2)-Math.sin(p1)*Math.cos(p2)*Math.cos(dl);return (toDeg(Math.atan2(y,x))+360)%360;
  }
  function displayHeading(pos){
    const h=Number(pos?.heading);if(Number.isFinite(h)&&h>=0)return h;
    if(state393.lastPosition&&distanceMeters(state393.lastPosition.lat,state393.lastPosition.lon,pos.lat,pos.lon)>3)return bearing(state393.lastPosition,pos);
    return Number.isFinite(state393.heading)?state393.heading:0;
  }
  ...
  function drawWalk(){
    const L=window.L,map=state393.map,p=state393.position,t=state393.target;if(!L||!map||!p||!t)return;clearWalk();
    const meters=distanceMeters(p.lat,p.lon,t.lat,t.lon),deg=bearing(p,t);state393.walkLine=L.polyline([[p.lat,p.lon],[t.lat,t.lon]],{weight:4,dashArray:'8 8',opacity:.9}).addTo(map);const mid=[(p.lat+t.lat)/2,(p.lon+t.lon)/2];state393.walkArrow=L.marker(mid,{icon:arrowIcon(L,deg,meters),interactive:false}).addTo(map);$("#dz393MapMeta")&&( $("#dz393MapMeta").textContent=`前往 ${t.name} · 約 ${Math.round(meters)}m` );
  }
  ...
  async function renderNearbyMap(){
    const host=mapHost();if(!state393.position)return;host.classList.remove("hidden");const L=await ensureLeaflet();
    if(!state393.map){const canvas=$("#dz393MapCanvas");state393.map=L.map(canvas,{zoomControl:true,attributionControl:true});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(state393.map);}
    const map=state393.map;state393.stopLayers.forEach(x=>map.removeLayer(x));state393.stopLayers=[];
    if(state393.radiusCircle)map.removeLayer(state393.radiusCircle);state393.radiusCircle=L.circle([state393.position.lat,state393.position.lon],{radius:state393.radius,weight:2,opacity:.5,fillOpacity:.05}).addTo(map);
    ...
    if(!state393.target||!state393.stops.some(x=>x.stop===state393.target.stop&&x.operator===state393.target.operator))state393.target=state393.stops[0]||null;
    drawWalk();focusRadius();setTimeout(()=>map.invalidateSize(),80);
  }
  async function runNearby(radius=state393.radius){
    ...
    navigator.geolocation.getCurrentPosition(async pos=>{
      ...
      const sec=$("#nearbySection");sec?.classList.remove('hidden');const count=$("#nearbyCount");if(count)count.textContent=`${state393.radius}m`;if(st)st.textContent=`已搜尋 ${state393.radius}m 範圍，共 ${state393.stops.length} 個附近站點。`;renderNearbyMap().catch(()=>{});startWatch();if(btn)btn.disabled=false;
  ...
  function startWatch(){
    if(state393.watchId!=null||!navigator.geolocation)return;state393.watchId=navigator.geolocation.watchPosition(pos=>{
      const next={lat:pos.coords.latitude,lon:pos.coords.longitude};state393.lastPosition=state393.position;state393.position=next;state393.heading=Number.isFinite(pos.coords.heading)?pos.coords.heading:state393.heading;if(!state393.map)return;const L=window.L;if(state393.userMarker&&L)state393.userMarker.setLatLng([next.lat,next.lon]).setIcon(userIcon(L,displayHeading({...next,heading:state393.heading})));if(state393.radiusCircle)state393.radiusCircle.setLatLng([next.lat,next.lon]);if(state393.target)drawWalk();
    },()=>{},{enableHighAccuracy:true,maximumAge:5000,timeout:15000});
  }
  document.addEventListener('click',e=>{
    const radius=e.target.closest?.('[data-dz-radius]');if(radius){e.preventDefault();e.stopImmediatePropagation();runNearby(Number(radius.dataset.dzRadius));return;}
    if(e.target.closest?.('#locateBtn')){e.preventDefault();e.stopImmediatePropagation();runNearby();return;}
    if(e.target.closest?.('#dz393Recenter')){e.preventDefault();focusRadius();return;}
    const card=e.target.closest?.('#nearbyResults .near-card');if(card&&state393.position){const route=card.querySelector('.near-route')?.textContent?.trim(),meta=card.querySelector('.near-meta')?.textContent||'',name=meta.split('·')[0].trim();const match=state393.stops.find(s=>s.name===name)||state393.stops[0];if(match){state393.target=match;drawWalk();}}
  },true);
  ensureControls();mapHost();
  window.dzNearby393={version:'3.9.3',runNearby,renderNearbyMap,collectStops};
})();