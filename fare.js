(() => {
  ...
  function routeFareRecords(xml, ctx = {}) {
    ...
    if (!route) return [];
    const candidates = [...xml.querySelectorAll("*")];
    const out = [];
    for (const n of candidates) {
      ...
      const cleanedFare = String(fareRaw || "").replace(/[^0-9.]/g, "");
      if (!cleanedFare) continue;
      const fare = Number(cleanedFare);
  ...
  function buildFareMap(records = []) {
    ...
    if (!map.size && records.length) {
      ...
      if (Number.isFinite(f) && f > 0) map.set(1, f);
    }
    return map;
  }
