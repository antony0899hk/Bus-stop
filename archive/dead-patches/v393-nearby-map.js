(() => {
  ...
  const state393={radius:100,position:null,heading:null,lastPosition:null,stops:[],target:null,map:null,userMarker:null,radiusCircle:null,walkLine:null,walkArrow:null,stopLayers:[],watchId:null,leafletPromise:null,tileLayer:null,mapToken:0,secondaryReady:false,secondaryPromise:null};
  ...
  function opMap(op){return op==='KMB'?state.kmbStops:op==='CTB'?state.ctbStops:op==='GMB'?state.gmbStops:null;}
  async function collectStopsSafeOps(pos,radius,operators){
    const out=[];
    const latPad=radius/110540+0.001,lonPad=radius/(111320*Math.max(.3,Math.cos(pos.lat*Math.PI/180)))+0.001;
    for(const operator of operators){
      const map=opMap(operator);if(!map)continue;
      const a=[];let n=0;
  ...
  function collectStops(pos,radius){
    const out=[];for(const operator of ['KMB','CTB','GMB']){const map=opMap(operator),a=[];for(const [id,s] of map||[]){const c=coords(s);if(!c)continue;const d=distanceMeters(pos.lat,pos.lon,c.lat,c.lon);if(Number.isFinite(d)&&d<=radius)a.push({operator,stop:String(id),stopObj:s,lat:c.lat,lon:c.lon,name:nameOf(s,id),distance:d});}a.sort((x,y)=>x.distance-y.distance);out.push(...a.slice(0,SAFE_CAP[operator]||4));}return out.sort((a,b)=>a.distance-b.distance);
  }
  async function collectStopsSafe(pos,radius){return collectStopsSafeOps(pos,radius,['KMB','CTB','GMB']);}
  function mergeNearbyRows(primary,secondary){
    const seen=new Set(),rows=[...(primary||[]),...(secondary||[])].sort((a,b)=>new Date(a.eta)-new Date(b.eta));
    return rows.filter(x=>{const k=`${String(x.operator||'')}|${String(x.route||'').toUpperCase()}`;if(seen.has(k))return false;seen.add(k);return true;});
  }
  async function loadEtaBatch(stops){
    if(typeof loadNearbyEtas!=='function')return [];
    await loadNearbyEtas(stops.map(x=>({operator:x.operator,stop:x.stop,stopObj:x.stopObj,distance:x.distance})));
    return [...(state.nearby||[])];
  }
  async function preloadMtr(){
    try{if(window.dzExtraTransit?.ensureMtrData)await window.dzExtraTransit.ensureMtrData();return true;}catch{return false;}
  }
  ...
  function installTiles(L,map,host,token){if(state393.tileLayer){try{map.removeLayer(state393.tileLayer);}catch{}}const primary=L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png',{subdomains:'abcd',maxZoom:20,attribution:'© OpenStreetMap © CARTO'});state393.tileLayer=primary;let done=false;const ok=()=>{if(done||token!==state393.mapToken)return;done=true;tileReady(host);};primary.once('tileload',ok);primary.addTo(map);setTimeout(()=>{if(done||token!==state393.mapToken)return;try{map.removeLayer(primary);}catch{}const fallback=L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'});state393.tileLayer=fallback;fallback.once('tileload',ok);fallback.addTo(map);setTimeout(ok,2200);},1800);}
  function startWatch(){if(state393.watchId!=null||!navigator.geolocation)return;state393.watchId=navigator.geolocation.watchPosition(pos=>{const next={lat:pos.coords.latitude,lon:pos.coords.longitude};state393.lastPosition=state393.position;state393.position=next;state393.heading=Number.isFinite(pos.coords.heading)?pos.coords.heading:state393.heading;if(!state393.map)return;const L=window.L;if(state393.userMarker&&L)state393.userMarker.setLatLng([next.lat,next.lon]).setIcon(userIcon(L,displayHeading({...next,heading:state393.heading})));if(state393.radiusCircle)state393.radiusCircle.setLatLng([next.lat,next.lon]);if(state393.target)drawWalk();},()=>{},{enableHighAccuracy:true,maximumAge:8000,timeout:15000});}
  async function renderNearbyMap(){const host=mapHost();if(!state393.position)return;host.classList.remove("hidden");host.classList.remove('map-ready');const row=$(".dz393-map-loading-row",host);if(row)row.textContent='地圖準備中…';const token=++state393.mapToken;const L=await ensureLeaflet();if(!state393.map){const canvas=$("#dz393MapCanvas");state393.map=L.map(canvas,{zoomControl:true,attributionControl:true});installTiles(L,state393.map,host,token);}else tileReady(host);const map=state393.map;state393.stopLayers.forEach(x=>map.removeLayer(x));state393.stopLayers=[];if(state393.radiusCircle)map.removeLayer(state393.radiusCircle);state393.radiusCircle=L.circle([state393.position.lat,state393.position.lon],{radius:state393.radius,weight:2,opacity:.5,fillOpacity:.05}).addTo(map);const h=displayHeading({...state393.position,heading:state393.heading});if(state393.userMarker){state393.userMarker.setLatLng([state393.position.lat,state393.position.lon]);state393.userMarker.setIcon(userIcon(L,h));}else state393.userMarker=L.marker([state393.position.lat,state393.position.lon],{icon:userIcon(L,h),zIndexOffset:1000}).addTo(map).bindTooltip('你的位置');for(const s of state393.stops){const m=L.marker([s.lat,s.lon],{icon:stopIcon(L,s)}).addTo(map).bindTooltip(`${s.name} · ${Math.round(s.distance)}m`);m.on('click',()=>{state393.target=s;drawWalk();});state393.stopLayers.push(m);}if(!state393.target||!state393.stops.some(x=>x.stop===state393.target.stop&&x.operator===state393.target.operator))state393.target=state393.stops[0]||null;drawWalk();focusRadius();startWatch();setTimeout(()=>map.invalidateSize(),80);}
  async function runNearby(radius=state393.radius){
    state393.radius=Number(radius)||100;state393.secondaryReady=false;state393.secondaryPromise=null;ensureControls();$$('[data-dz-radius]').forEach(b=>b.classList.toggle('active',Number(b.dataset.dzRadius)===state393.radius));const title=$(".nearby-panel .panel-title");if(title)title.textContent=`📍 ${state393.radius}m 即將到站`;const st=$("#nearbyStatus");if(st)st.textContent='正在取得位置…';const btn=$("#locateBtn");if(btn)btn.disabled=true;
    ...
    navigator.geolocation.getCurrentPosition(async pos=>{
      try{
        ...
        if(st)st.textContent=`定位成功，先載入巴士…`;
        const primaryStops=await collectStopsSafeOps(p,state393.radius,['KMB','CTB']);
        state393.stops=[...primaryStops];
        const primaryRows=await loadEtaBatch(primaryStops);
        const sec=$("#nearbySection");sec?.classList.remove('hidden');const count=$("#nearbyCount");if(count)count.textContent=`${state393.radius}m`;
        if(st)st.textContent=`巴士已顯示；小巴及 MTR 背景載入中…`;
        if(btn)btn.disabled=false;
        state393.secondaryPromise=(async()=>{
          await YIELD();
          const [gmbStops]=await Promise.all([collectStopsSafeOps(p,state393.radius,['GMB']),preloadMtr()]);
          const gmbRows=await loadEtaBatch(gmbStops);
          state393.stops=[...primaryStops,...gmbStops].sort((a,b)=>a.distance-b.distance);
          state.nearby=mergeNearbyRows(primaryRows,gmbRows);
          try{if(typeof renderNearby==='function')renderNearby();}catch{}
          state393.secondaryReady=true;
          if(st)st.textContent=`已完成 ${state393.radius}m：巴士先出，小巴及 MTR 已準備完成。`;
          document.dispatchEvent(new CustomEvent('dz:nearby-secondary-ready',{detail:{radius:state393.radius,position:p}}));
          return true;
        })();
      }catch{if(st)st.textContent='附近資料暫時未能載入。';if(btn)btn.disabled=false;}
  ...
  window.dzNearby393={version:'3.10.3',runNearby,renderNearbyMap,collectStops,collectStopsSafe,startWatch,waitSecondary:()=>state393.secondaryPromise||Promise.resolve(state393.secondaryReady)};
})();